...
- **Deterministic node IDs:** `Node.id` is now derived from `(file_path, name, node_type)` via `StableNodeKey` + UUID v5 when a file path is set. Cross-file call edges survive single-file re-extract + compaction. **Migration:** delete `.rgctl` / `.rgctl-base` and run `rgctl discover .` once (see [ci-policy-checks.md](../guides/ci-policy-checks.md#deterministic-node-ids-migration)).
