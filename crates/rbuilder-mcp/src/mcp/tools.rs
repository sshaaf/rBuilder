...
use rbuilder_analysis::community::{CommunityDetector, CommunityResult};
use rbuilder_analysis::complexity::{ComplexityAnalyzer, ComplexityReport};
use rbuilder_analysis::dependency::DependencyAnalyzer;
...
use rbuilder_nlp::dual_agent::DualAgentQuerySystem;
...
use rbuilder_semantic::SignatureExtractor;
...
impl ToolExecutor {
    ...
    fn find_ansible_roles(
        ...
        Err(Error::Other(
            "find_ansible_roles requires IaC modules in the rbuilder binary crate".into(),
        ))
    }
    ...
    fn ansible_security_scan(
        ...
        Err(Error::Other(
            "ansible_security_scan requires IaC modules in the rbuilder binary crate".into(),
        ))
    }
    ...
    fn find_chef_recipes(
        &self,
        _backend: &MemoryBackend,
        _cookbook_filter: Option<&str>,
        _verbose: bool,
    ) -> Result<Value> {
        Err(Error::Other(
            "find_chef_recipes requires IaC modules in the rbuilder binary crate".into(),
        ))
    }
    ...
    fn chef_security_scan(
        ...
        Err(Error::Other(
            "chef_security_scan requires IaC modules in the rbuilder binary crate".into(),
        ))
    }
    ...
    fn find_puppet_classes(
        ...
        Err(Error::Other(
            "find_puppet_classes requires IaC modules in the rbuilder binary crate".into(),
        ))
    }
    ...
    fn puppet_security_scan(
        ...
        Err(Error::Other(
            "puppet_security_scan requires IaC modules in the rbuilder binary crate".into(),
        ))
    }
    ...
    fn symbol_info(
        ...
        let level = format!(
            "{:?}",
            rbuilder_analysis::complexity::classify_complexity(cyclomatic)
        );
    ...
    fn diff_analysis(&self, since: Option<&str>, verbose: bool) -> Result<Value> {
        ...
        let git_files: Vec<String> = if let Some(ref commit) = since_ref {
            git_changed_files(&self.repo_root, commit)?
                .into_iter()
                .filter_map(|p| {
                    rbuilder_incremental::file_tracker::relative_path(&self.repo_root, &p).ok()
                })
                .collect()
        } else {
            Vec::new()
