...
//!
//! ```no_run
//! use rbuilder_lang_chef::CookbookDependencyGraph;
    ...
    pub fn analyze_cookbooks_dir(&self, cookbooks_path: &Path) -> Result<CookbookDependencyGraph> {
        ...
        for entry in std::fs::read_dir(cookbooks_path)? {
            ...
            if meta.exists() {
                ...
                let (symbols, relations) = crate::parse_content(&meta.to_string_lossy(), &content);
                for sym in symbols {
                    if sym.symbol_type == rbuilder_plugin_api::SymbolType::ChefCookbook
                ...
                for rel in relations {
                    if rel.relation_type == rbuilder_plugin_api::RelationType::DependsOnCookbook {
                        let dep = rel
                            .to
                            .strip_prefix("cookbook::")
                            .unwrap_or(&rel.to)
