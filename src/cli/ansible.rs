...
use crate::analysis::ansible_roles::{RoleDependencyAnalyzer, RoleDependencyGraph};
...
fn validate_ansible_path(path: &Path) -> Result<()> {
    ...
    if path.is_dir() {
        walk_yaml_files(path, &mut |file_path| {
            if !AnsibleParser::is_ansible_path(&file_path.to_string_lossy()) {
                return Ok(());
            }
            let content = std::fs::read_to_string(&file_path)?;
            let value: serde_yaml::Value = serde_yaml::from_str(&content).unwrap_or_default();
            let (symbols, _) = parser.parse(&file_path.to_string_lossy(), &value, &content);
            let plays = symbols
                .iter()
                .filter(|s| {
...
fn scan_playbooks_on_disk(path: &Path) -> Result<Vec<AnsibleSecurityFinding>> {
    ...
    let files: Vec<PathBuf> = if path.is_file() {
        ...
        walk_yaml_files(path, &mut |p| {
            if AnsibleParser::is_ansible_path(&p.to_string_lossy()) {
                collected.push(p);
            }
            Ok(())
        })?;
    ...
    for file_path in files {
        ...
        let (symbols, _) = parser.parse(&file_path.to_string_lossy(), &value, &content);
        for sym in symbols {
            if sym.symbol_type != crate::languages::plugin_trait::SymbolType::AnsibleTask
                && sym.symbol_type != crate::languages::plugin_trait::SymbolType::AnsibleHandler
            {
                continue;
            }
