...
use crate::analysis::puppet_modules::{ModuleDependencyAnalyzer, ModuleDependencyGraph};
use crate::error::{Error, Result};
use crate::graph::CodeGraph;
use crate::languages::multimodal::puppet::parser::PuppetParser;
use crate::security::puppet::{PuppetSecurityFinding, PuppetSecurityScanner, PuppetSeverity};
...
fn validate_puppet_path(path: &Path) -> Result<()> {
    ...
    if path.extension().and_then(|e| e.to_str()) == Some("pp")
        && PuppetParser::is_puppet_path(&path.to_string_lossy())
    {
        let content = std::fs::read_to_string(path)?;
        let (symbols, _) = parser.parse(&path.to_string_lossy(), &content);
        let classes = symbols
            .iter()
            .filter(|s| s.symbol_type == crate::languages::plugin_trait::SymbolType::PuppetClass)
            .count();
        if classes == 0 {
    ...
    if path.is_dir() {
        walk_pp_files(path, &mut |file_path| {
            if !PuppetParser::is_puppet_path(&file_path.to_string_lossy()) {
                return Ok(());
            }
...
fn scan_modules_on_disk(path: &Path) -> Result<Vec<PuppetSecurityFinding>> {
    ...
    walk_pp_files(path, &mut |file_path| {
        ...
        let (symbols, _) = parser.parse(&file_path.to_string_lossy(), &content);
        for sym in symbols {
            if sym.symbol_type != crate::languages::plugin_trait::SymbolType::PuppetResource {
                continue;
            }
            let mut node = crate::graph::schema::Node::new(
                crate::graph::schema::NodeType::PuppetResource,
                sym.name,
            );
