...
//!
//! Task 3.2.2: Load external language plugins from shared libraries.

use crate::error::{Error, Result};
use crate::languages::plugin_abi::{PluginMetadata, PLUGIN_ABI_VERSION};
...
/// Plugin loader for external shared libraries.
...
impl PluginLoader {
    ...
    #[cfg(feature = "plugin-system")]
    fn inspect_dynamic(path: &Path) -> Result<PluginMetadata> {
        use libloading::Library;

        unsafe {
            let lib = Library::new(path).map_err(|e| Error::PluginError(e.to_string()))?;
            let abi_fn: libloading::Symbol<unsafe extern "C" fn() -> u32> = lib
                .get(crate::languages::plugin_abi::exports::PLUGIN_ABI.as_bytes())
                .map_err(|e| Error::PluginError(e.to_string()))?;
            let abi_version = abi_fn();
            if abi_version != PLUGIN_ABI_VERSION {
                return Err(Error::PluginError(format!(
                    "Unsupported plugin ABI version: {abi_version}"
                )));
            }
